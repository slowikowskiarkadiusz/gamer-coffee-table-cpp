...
tetris_board_actor::tetris_board_actor(
    v2 center, int seed, bool is_p1,
    std::function<void(int, bool)> on_deal_damage,
    std::function<void(bool)> on_death)
    : actor(is_p1 ? "p1_tetris-board" : "p2_tetris-board", center,
            v2(1 + board_width + 1 + 1 + 1,
               1 + board_height + 1 + hold_logic::default_size.y + 1)),
      center(center),
      seed(seed),
      is_p1(is_p1),
      on_deal_damage(on_deal_damage),
      on_death(on_death),
      garbage_bar_logic_(v2(board_width + 2, size().y - 1 - board_height / 2),
                         v2(1, board_height), {color::white(0.1f)}),
      hold_logic_(v2::zero()),
      ...
      render_matrix(size().x, size().y)
{
    ...
    write_border(v2(0, size().y - 1),
                 v2(board_width + 1, size().y - 1 - board_height - 1));
    write_border(v2(board_width + 1, size().y - 1),
                 v2(board_width + 1 + garbage_bar_logic_.size.x + 1,
                    size().y - 1 - board_height - 1));

    hold_logic_.center =
        v2(hold_logic_.get_size().x / 2 + 1,
           size().y - 1 - board_height - 1 - hold_logic_.get_size().y / 2);
    write_border(
        v2(0, size().y - 1 - board_height - 1 - hold_logic_.get_size().y - 1),
...
void tetris_board_actor::update(float deltaTime)
{
    ...
    {
        if (gestures::is(
                is_p1 ? std::vector{key::P1_BLUE} : std::vector{key::P2_BLUE},
                state::press, gesture::prolonged) &&
            !switched_pieces)
        {
        ...
        else if (gestures::is(is_p1 ? std::vector{key::P1_DOWN}
                                    : std::vector{key::P2_DOWN},
                              state::press, gesture::once))
            dropping_delay_value = faster_dropping_delay;
        else
        {
            dropping_delay_value = dropping_delay;
            if (gestures::is(
                    is_p1 ? std::vector{key::P1_LEFT} : std::vector{key::P2_LEFT},
                    state::down, gesture::once) ||
                gestures::is(
                    is_p1 ? std::vector{key::P1_LEFT} : std::vector{key::P2_LEFT},
                    state::press, gesture::repeating))
                move_block_by(v2::left());

            if (gestures::is(
                    is_p1 ? std::vector{key::P1_RIGHT} : std::vector{key::P2_RIGHT},
                    state::down, gesture::once) ||
                gestures::is(
                    is_p1 ? std::vector{key::P1_RIGHT} : std::vector{key::P2_RIGHT},
                    state::press, gesture::repeating))
        ...
        if (gestures::is(is_p1 ? std::vector{key::P1_UP} : std::vector{key::P2_UP},
                         state::down, gesture::once))
            drop();
        ...
        if (gestures::is(
                is_p1 ? std::vector{key::P1_GREEN} : std::vector{key::P2_GREEN},
                state::down, gesture::once))
            rotate_block(-1);
    ...
    v2 boardOffset(1 + board_width / 2,
                   size().y - 1 - board_height + board_height / 2);

    render_matrix.write_at_origin(borders_matrix, v2::zero());
    render_matrix.write(garbage_bar_logic_.render(), garbage_bar_logic_.center,
                        0);
...
matrix *tetris_board_actor::render() { return &render_matrix; }

void tetris_board_actor::stop() { do_play = false; }

void tetris_board_actor::spawn(std::optional<v2> center,
                               std::optional<tetris_shape> shape)
{
...
void tetris_board_actor::rotate_block(int dir)
{
    if (!current_agent || !current_agent_shadow)
        return;
    auto kicks =
        current_agent->get_kicks(current_agent->get_block_rotation() + 90 * dir);
...
void tetris_board_actor::drop()
{
    ...
    for (const auto &spot : current_agent->get_taken_spots())
    {
        ...
        static_board_matrix.set_pixel(
            spot.x, spot.y,
            tetris_block_colors[static_cast<int>(current_agent->get_shape())]);
...
int tetris_board_actor::clear_lines()
{
    ...
    {
        if (std::all_of(is_taken.begin(), is_taken.end(),
                        [y](const std::vector<bool> &col)
                        { return col[y]; }))
            lines.push_back(y);
    }
    if (lines.empty())
            ...
            for (int line : lines)
            {
                for (int y = line; y > 0; --y)
                {
                    for (int x = 0; x < board_width; ++x)
                    {
                        ...
                        static_board_matrix.set_pixel(
                            x, y, static_board_matrix.pixels.at(x, y - 1));
                    }
                }
            }
        },
...
int tetris_board_actor::pop_garbage_lines()
{
    ...
    while (garbage_bar_logic_.pop())
    {
        for (int y = 0; y < board_height - 1; ++y)
        {
            ...
            {
                is_taken[x][y] = is_taken[x][y + 1];
                static_board_matrix.set_pixel(x, y,
                                              static_board_matrix.pixels.at(x, y + 1));
            }
        }
        ...
        for (int x = 0; x < board_width; ++x)
        {
            ...
            static_board_matrix.set_pixel(
                x, board_height - 1, x != hole ? color::white(0.5f) : color::none());
        }
    }
