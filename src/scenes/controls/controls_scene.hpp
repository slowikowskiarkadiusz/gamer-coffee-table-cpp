...
public:
    explicit controls_scene(std::function<std::shared_ptr<scene>()> next_scene_factory, std::string next_scene_name)
        ...
        lines_per_page = (engine::screen_size.y / 2 - 5) / (font_size + 1);
    }
...
private:
    std::vector<std::vector<controls_data> > paginate(const std::vector<controls_data> &items) {
        std::vector<std::vector<controls_data> > result;
        size_t i = 0;
        while (i < items.size()) {
            result.emplace_back(items.begin() + i, items.begin() + std::min(i + lines_per_page, items.size()));
            i += lines_per_page;
        }
        return result;
    }
    ...
    void print_page() {
        ...
        for (int is_p1 = 0; is_p1 <= 1; ++is_p1) {
            std::vector<std::shared_ptr<actor> > current_player_actors;
            for (int i = 0; i < current.size(); ++i) {
                float y = engine::screen_size.y / 2 - (button_size + 1) * (i + 1) + (is_p1 ? 0 : engine::screen_size.y / 2);
                float x = 0;
                auto currLine = current[current.size() - 1 - i];
                for (key k: currLine.keys) {
                    ...
                    current_player_actors.push_back(icon);
                    x += (button_size + 1);

                    auto operation = currLine.operation;
                    if (!operation.empty() && k != currLine.keys[currLine.keys.size() - 1]) {
                        auto text = engine::instantiate<text_actor>(operation, v2(x, y - (button_size / 2)), v2(operation.size() * 4, 5));
                        current_page_actors.push_back(text);
                        current_player_actors.push_back(text);
                        x += text->size().x;
                    }
                }
                x += 2;
            ...
            if (is_p1) {
                auto rot_center = engine::screen_size / 2 - v2::one();
                rot_center.y -= engine::screen_size.y / 4;
                for (auto &actor: current_player_actors)
                    actor->rotate_around(rot_center, 180);
            }
        }

        engine::set_timeout([this, current_page_actors ]() {
            for (auto &a: current_page_actors) {
                a->kill();
            }
